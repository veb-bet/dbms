...
class Ui_MainWindow(QWidget):
    ...
    def setupUi(self):
        ...
        insert_btn2.clicked.connect(self.insert_into_client)
    ...
    def insert_into_results(conn):
        ...
        if ok1 and ok2 and ok3:
            ...
            if conn:
                ...
                try:
                    cursor.execute("INSERT INTO client.Results (idresults, dateresult, description) VALUES (%s, %s, %s)",
    ...
    def update_client(conn):
        ...
        if conn:
            idclient, ok1 = QInputDialog.getText(None, "Enter Data", "idclient:", QLineEdit.Normal, "")
            new_nameclient, ok2 = QInputDialog.getText(None, "Enter Data", "new nameclient:", QLineEdit.Normal, "")
            ...
            cursor = conn.cursor()
            cursor.execute("UPDATE client.Client SET nameclient = %s, phone = %s, email = %s WHERE idclient = %s",
                       (new_nameclient, new_phone, new_email, idclient))
            conn.commit()
            cursor.close()
            conn.close()
            ...
            if ok1 and ok2 and ok3 and ok4 and ok5 and ok6 and ok7 and ok8 and ok9 and ok10:
                ...
                try:
                    cursor.execute(
                        ...
                        (new_nameTherapist, new_email, new_phone, new_hospitalId, new_educationId, new_coursesId, new_experienceId, new_questionsId, new_costId, idTherapist))
                    conn.commit()
                except Exception as e:
                    print('Ошибка в запросе: ', e)
                    conn.rollback()
    ...
    def insert_into_hospital(conn):
        ...
        if ok1 and ok2 and ok3:
            ...
            if conn:
                ...
                try:
                    cursor.execute("INSERT INTO therapists.hospital (idHospital, nameHospital, address) VALUES (%s, %s, %s)",
